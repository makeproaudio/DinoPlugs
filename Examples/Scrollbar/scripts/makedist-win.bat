...
if %1 == 1 (echo Making Scrollbar Windows DEMO VERSION distribution ...) else (echo Making Scrollbar Windows FULL VERSION distribution ...)
...
REM - msbuild Scrollbar-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Scrollbar.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Scrollbar.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Scrollbar.aaxplugin\Contents\Win32\Scrollbar.aaxplugin --out .\build-win\aax\bin\Scrollbar.aaxplugin\Contents\Win32\Scrollbar.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Scrollbar.aaxplugin\Contents\x64\Scrollbar.aaxplugin --out .\build-win\aax\bin\Scrollbar.aaxplugin\Contents\x64\Scrollbar.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Scrollbar.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Scrollbar.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Scrollbar Installer" ".\installer\Scrollbar Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Scrollbar Installer.exe" ".\installer\Scrollbar Demo Installer.exe"
REM -del ".\installer\Scrollbar Installer.exe"
REM -)
