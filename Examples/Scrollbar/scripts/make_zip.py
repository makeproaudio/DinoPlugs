...
def  main():
  ...
  installer = "\installer\Scrollbar Installer.exe"
   
  if demo:
    installer = "\installer\Scrollbar Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Scrollbar manual.pdf" 
  ]
  ...
  ZIPNAME = "Scrollbar-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Scrollbar-v" + FULLVERSIONSTR + "-win-demo.zip"
