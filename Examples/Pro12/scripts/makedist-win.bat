...
if %1 == 1 (echo Making Pro12 Windows DEMO VERSION distribution ...) else (echo Making Pro12 Windows FULL VERSION distribution ...)
...
REM - msbuild Pro12-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Pro12.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Pro12.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Pro12.aaxplugin\Contents\Win32\Pro12.aaxplugin --out .\build-win\aax\bin\Pro12.aaxplugin\Contents\Win32\Pro12.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Pro12.aaxplugin\Contents\x64\Pro12.aaxplugin --out .\build-win\aax\bin\Pro12.aaxplugin\Contents\x64\Pro12.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Pro12.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Pro12.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Pro12 Installer" ".\installer\Pro12 Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Pro12 Installer.exe" ".\installer\Pro12 Demo Installer.exe"
REM -del ".\installer\Pro12 Installer.exe"
REM -)
