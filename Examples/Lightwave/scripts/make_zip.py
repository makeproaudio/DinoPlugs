...
def  main():
  ...
  installer = "\installer\Lightwave Installer.exe"
   
  if demo:
    installer = "\installer\Lightwave Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Lightwave manual.pdf" 
  ]
  ...
  ZIPNAME = "Lightwave-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Lightwave-v" + FULLVERSIONSTR + "-win-demo.zip"
