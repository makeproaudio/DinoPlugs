...
def  main():
  ...
  installer = "\installer\B4000 Installer.exe"
   
  if demo:
    installer = "\installer\B4000 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\B4000 manual.pdf" 
  ]
  ...
  ZIPNAME = "B4000-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "B4000-v" + FULLVERSIONSTR + "-win-demo.zip"
