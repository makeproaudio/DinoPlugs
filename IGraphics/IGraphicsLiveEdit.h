...
{
public:
  IGraphicsLiveEdit(bool mouseOversEnabled, const char* pathToSourceFile = 0, float gridSize = 1)
    : IControl(IRECT())
  ...
  void Draw(IGraphics& g) override
  {
    ...
    for (int i = 1; i < g.NControls(); i++)
    {
      ...
      if (myfile.is_open())
      {
        myfile << "IRECT(" << cr.L << "," << cr.T << "," << cr.R << "," << cr.B << ")" << std::endl;
      }
