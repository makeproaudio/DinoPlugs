...
def  main():
  ...
  installer = "\installer\Prodyssey Installer.exe"
   
  if demo:
    installer = "\installer\Prodyssey Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Prodyssey manual.pdf" 
  ]
  ...
  ZIPNAME = "Prodyssey-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Prodyssey-v" + FULLVERSIONSTR + "-win-demo.zip"
