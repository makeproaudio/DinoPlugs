...
def  main():
  ...
  installer = "\installer\Minimax Installer.exe"
   
  if demo:
    installer = "\installer\Minimax Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Minimax manual.pdf" 
  ]
  ...
  ZIPNAME = "Minimax-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Minimax-v" + FULLVERSIONSTR + "-win-demo.zip"
