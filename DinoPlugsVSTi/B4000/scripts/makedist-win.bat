...
if %1 == 1 (echo Making B4000 Windows DEMO VERSION distribution ...) else (echo Making B4000 Windows FULL VERSION distribution ...)
...
REM - msbuild B4000-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild B4000.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild B4000.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\B4000.aaxplugin\Contents\Win32\B4000.aaxplugin --out .\build-win\aax\bin\B4000.aaxplugin\Contents\Win32\B4000.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\B4000.aaxplugin\Contents\x64\B4000.aaxplugin --out .\build-win\aax\bin\B4000.aaxplugin\Contents\x64\B4000.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\B4000.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\B4000.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "B4000 Installer" ".\installer\B4000 Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\B4000 Installer.exe" ".\installer\B4000 Demo Installer.exe"
REM -del ".\installer\B4000 Installer.exe"
REM -)
