...
def  main():
  ...
  installer = "\installer\DrumsNBass Installer.exe"
   
  if demo:
    installer = "\installer\DrumsNBass Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DrumsNBass manual.pdf" 
  ]
  ...
  ZIPNAME = "DrumsNBass-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DrumsNBass-v" + FULLVERSIONSTR + "-win-demo.zip"
