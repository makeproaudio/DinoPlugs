...
def  main():
  ...
  installer = "\installer\Pro12 Installer.exe"
   
  if demo:
    installer = "\installer\Pro12 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Pro12 manual.pdf" 
  ]
  ...
  ZIPNAME = "Pro12-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Pro12-v" + FULLVERSIONSTR + "-win-demo.zip"
