...
def  main():
  ...
  installer = "\installer\FMagia Installer.exe"
   
  if demo:
    installer = "\installer\FMagia Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FMagia manual.pdf" 
  ]
  ...
  ZIPNAME = "FMagia-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FMagia-v" + FULLVERSIONSTR + "-win-demo.zip"
